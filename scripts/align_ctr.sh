...
for sample in `ls ~/reciprocal_t/data/trimmed | grep '.fq.gz' | cut -f 1-3 -d "_"| uniq | grep 'AAAA\|HHHH' | grep -v 'F1'
`
...
do
    ...
    ~/bin/bamtools/bamtools merge -in ~/tonsa_f20/data/aligned/${lib1}.bam -in ~/tonsa_f20/data/aligned/${lib2}.bam | $my_samtools sort - -O bam -o ~/tonsa_f20/data/aligned/${sample}.bam
